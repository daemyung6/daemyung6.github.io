window.addEventListener('DOMContentLoaded', function () {
    const barGraph = document.querySelector('main .statistics.widget .bar-graph-box .front');
    const barNum = 15;

    for (let i = 0; i < barNum; i++) {
        barGraph.appendChild((function() {
            let div = document.createElement('div');
            div.classList.add('bar');
            div.style.height = (Math.random() * 100) + '%';
            div.style.width = `calc(100% / ${barNum + 2})`;
 ...
 */
    ...
    var update = function () {
        ...
        chart.config.options.scales.yAxes = [{
            display: true,
            gridLines: {
                display: false
            }
